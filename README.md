...
**Embedding 模型资源对比**（CPU 模式实测）：

| 模型 | 内存 | 单次检索 | 准确率 |
|---|---|---|---|
| **Qwen3-0.6B**（默认） | 228MB | 93ms | **5/5** |
| bge-base-zh-v1.5 | **33MB** | **74ms** | 3/5 |
| Gemma-300M | 292MB | 101ms | 3/5 |
| Qwen3-GGUF Q8_0 | ~1.4GB | 稍快 | 3/4（量化损失） |

> 结论：Qwen3-0.6B 效果最优且资源可接受；bge-base-zh 最省资源但效果差一档；GGUF 因 llama.cpp 缓冲反而不省内存。

**存储优化（DiskANN/IVF-PQ）**：10 万条向量暴力扫描仅 4.8ms，几百条记忆 0.2ms——瓶颈在模型推理（93ms）不在检索，**DiskANN/IVF-PQ 完全没必要**（那是百万级才需要的）。
